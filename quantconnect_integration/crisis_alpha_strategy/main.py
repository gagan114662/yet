...
class CrisisAlphaStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # VIX proxy using VXX for volatility analysis
        # VXX already added above as volatility instrument
        self.vix_proxy = self.vxx  # Use VXX as VIX proxy
        
        # SPY volatility calculation for crisis detection
    ...
    def DetectCrisisConditions(self, data):
        ...
        # 1. VIX Spike Detection using VXX proxy
    ...
    def CalculateVixFromVXX(self):
        ...
        if self.vix_proxy in self.Securities:
            ...
            # Convert VXX to VIX-like scale
            vix_from_vxx = vxx_price * 2.5  # Calibrated conversion factor
            
            # Method 2: Calculate from SPY volatility for crisis detection
            vix_from_spy = self.CalculateImpliedVolFromSPY()
            
            # During crisis, prefer SPY-based calculation as it's more responsive
            if self.crisis_mode and vix_from_spy > 0:
                combined_vix = (vix_from_vxx * 0.4) + (vix_from_spy * 0.6)
            elif vix_from_spy > 0:
                combined_vix = (vix_from_vxx * 0.7) + (vix_from_spy * 0.3)
            else:
                combined_vix = vix_from_vxx
                
            return max(10, min(100, combined_vix))  # Allow higher VIX during crisis
        else:
            return self.CalculateImpliedVolFromSPY()
    
    def CalculateImpliedVolFromSPY(self):
        """Calculate implied VIX from SPY returns for crisis detection"""
        if self.spy in self.Securities:
            ...
            # Calculate realized volatility with crisis adjustment
            if self.spy_returns_window.IsReady:
                returns = [self.spy_returns_window[i] for i in range(self.spy_returns_window.Count)]
                if len(returns) > 5:
                    import numpy as np
                    volatility = np.std(returns) * np.sqrt(252)
                    # Convert to VIX-like scale with higher risk premium for crisis
                    risk_premium = 1.5 if self.crisis_mode else 1.3
                    implied_vix = volatility * 100 * risk_premium
                    
                    self.calculated_vix = max(10, min(80, implied_vix))
