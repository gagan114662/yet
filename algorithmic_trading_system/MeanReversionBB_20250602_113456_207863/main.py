...
class RDAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        # Risk management
        self.SetRiskManagement(MaximumDrawdownPercentPerSecurity(0.031))
    ...
    def Rebalance(self):
        ...
        for symbol in self.indicators:
            if not self.IsWarmingUp:
                ...
                if signal > 0:
                    insights.append(Insight.Price(symbol, timedelta(days=11), InsightDirection.Up))
                elif signal < 0:
                    insights.append(Insight.Price(symbol, timedelta(days=11), InsightDirection.Down))
