...
class RDAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.lookback_period = 43
        self.rebalance_frequency = 3
        self.position_size = 0.151
        ...
        # Risk management
        self.SetRiskManagement(MaximumDrawdownPercentPerSecurity(0.073))
    ...
    def Rebalance(self):
        ...
        for symbol in self.indicators:
            if not self.IsWarmingUp:
                ...
                if signal > 0:
                    insights.append(Insight.Price(symbol, timedelta(days=8), InsightDirection.Up))
                elif signal < 0:
                    insights.append(Insight.Price(symbol, timedelta(days=8), InsightDirection.Down))
