...
class RDAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.lookback_period = 54
        self.rebalance_frequency = 3
        self.position_size = 0.158
        ...
        # Risk management
        self.SetRiskManagement(MaximumDrawdownPercentPerSecurity(0.071))
    ...
    def Rebalance(self):
        ...
        for symbol in self.indicators:
            if not self.IsWarmingUp:
                ...
                if signal > 0:
                    insights.append(Insight.Price(symbol, timedelta(days=19), InsightDirection.Up))
                elif signal < 0:
                    insights.append(Insight.Price(symbol, timedelta(days=19), InsightDirection.Down))
