...
class RDAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.lookback_period = 22
        self.rebalance_frequency = 7
        self.position_size = 0.172
        ...
        # Risk management
        self.SetRiskManagement(MaximumDrawdownPercentPerSecurity(0.046))
    ...
    def Rebalance(self):
        ...
        for symbol in self.indicators:
            if not self.IsWarmingUp:
                ...
                if signal > 0:
                    insights.append(Insight.Price(symbol, timedelta(days=12), InsightDirection.Up))
                elif signal < 0:
                    insights.append(Insight.Price(symbol, timedelta(days=12), InsightDirection.Down))
