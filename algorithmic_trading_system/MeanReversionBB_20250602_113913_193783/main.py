...
class RDAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.lookback_period = 53
        self.rebalance_frequency = 3
        self.position_size = 0.06
    ...
    def Rebalance(self):
        ...
        for symbol in self.indicators:
            if not self.IsWarmingUp:
                ...
                if signal > 0:
                    insights.append(Insight.Price(symbol, timedelta(days=9), InsightDirection.Up))
                elif signal < 0:
                    insights.append(Insight.Price(symbol, timedelta(days=9), InsightDirection.Down))
