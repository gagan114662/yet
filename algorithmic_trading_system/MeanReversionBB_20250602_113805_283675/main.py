...
class RDAgentStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.lookback_period = 13
        self.rebalance_frequency = 10
        self.position_size = 0.143
        ...
        # Risk management
        self.SetRiskManagement(MaximumDrawdownPercentPerSecurity(0.036))
    ...
    def Rebalance(self):
        ...
        for symbol in self.indicators:
            if not self.IsWarmingUp:
                ...
                if signal > 0:
                    insights.append(Insight.Price(symbol, timedelta(days=17), InsightDirection.Up))
                elif signal < 0:
                    insights.append(Insight.Price(symbol, timedelta(days=17), InsightDirection.Down))
