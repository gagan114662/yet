...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen8_agent1
    Type: HYBRID | Asset: BITO | Leverage: 12.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(12.430393792158405)
        ...
        self.rsi = self.rsi('BITO', 12)
        self.bb = self.bb('BITO', 20, 2)
        
        # Risk management
        self.stop_loss = 0.0769858882400417
        self.take_profit = 0.17851891290811567
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.2827021821896654 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("BITO", 1.2827021821896654 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.0769858882400417:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.17851891290811567:
                self.liquidate()
