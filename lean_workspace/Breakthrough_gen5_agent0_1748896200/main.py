...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen5_agent0
    Type: MEAN_REVERSION | Asset: QQQ | Leverage: 16.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(16.26821968719215)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('QQQ', 6)
        self.sma_slow = self.sma('QQQ', 31)
        self.macd = self.macd('QQQ', 12, 26, 9)
        self.bb = self.bb('QQQ', 20, 2)
        
        # Risk management
        self.stop_loss = 0.05811791685814059
        self.take_profit = 0.0516768192563232
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 1.5729279995020222
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05811791685814059:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.0516768192563232:
                self.liquidate()
