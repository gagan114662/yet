...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen8_agent2
    Type: BREAKOUT | Asset: SPY | Leverage: 8.6x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(8.639395609282953)
        ...
        self.sma_slow = self.sma('SPY', 32)
        self.rsi = self.rsi('SPY', 19)
        self.bb = self.bb('SPY', 20, 2)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.029787909271634876
        self.take_profit = 0.08547563706477126
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.0396002769984138
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.029787909271634876:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.08547563706477126:
                self.liquidate()
