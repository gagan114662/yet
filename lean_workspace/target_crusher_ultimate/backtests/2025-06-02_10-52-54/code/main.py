...
class TargetCrusherUltimate(QCAlgorithm):
    ...
    def WeeklyRiskAssessment(self):
        ...
        if "SPY" in self.indicators and "std_20" in self.indicators["SPY"] and self.indicators["SPY"]["std_20"].IsReady and "SPY" in self.Securities:
            spy_volatility = self.indicators["SPY"]["std_20"].Current.Value / self.Securities["SPY"].Price
        else:
            spy_volatility = 0.02  # Default moderate volatility
