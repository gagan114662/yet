...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen3_agent4
    Type: BREAKOUT | Asset: TQQQ | Leverage: 9.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(9.43269237271046)
        ...
        self.sma_slow = self.sma('TQQQ', 50)
        self.macd = self.macd('TQQQ', 12, 26, 9)
        self.atr = self.atr('TQQQ', 14)
        
        # Risk management
        self.stop_loss = 0.09328528767287941
        self.take_profit = 0.16922951459037466
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.182428942848245
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.09328528767287941:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.16922951459037466:
                self.liquidate()
