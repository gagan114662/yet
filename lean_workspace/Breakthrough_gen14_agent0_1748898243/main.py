...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen14_agent0
    Type: HYBRID | Asset: QQQ | Leverage: 7.7x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(7.658411743609823)
        ...
        self.sma_slow = self.sma('QQQ', 34)
        self.rsi = self.rsi('QQQ', 17)
        self.macd = self.macd('QQQ', 12, 26, 9)
        self.bb = self.bb('QQQ', 20, 2)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.06528895071887612
        self.take_profit = 0.05149122388437486
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 2.924549794986108 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("QQQ", 2.924549794986108 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.06528895071887612:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.05149122388437486:
                self.liquidate()
