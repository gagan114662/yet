...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen6_agent2
    Type: MOMENTUM | Asset: UPRO | Leverage: 6.7x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(6.743852264236325)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('UPRO', 14)
        self.sma_slow = self.sma('UPRO', 36)
        self.rsi = self.rsi('UPRO', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.05147392473247264
        self.take_profit = 0.16397898523823068
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.009514634867281
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05147392473247264:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.16397898523823068:
                self.liquidate()
