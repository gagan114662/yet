...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen9_agent4
    Type: MEAN_REVERSION | Asset: UPRO | Leverage: 7.9x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(7.9021294938800395)
        ...
        # Risk management
        self.stop_loss = 0.08928653741311036
        self.take_profit = 0.08294067199792816
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.5566135453487036
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08928653741311036:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.08294067199792816:
                self.liquidate()
