...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen13_agent2
    Type: HYBRID | Asset: QQQ | Leverage: 12.8x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(12.776399361779434)
        ...
        self.rsi = self.rsi('QQQ', 12)
        self.bb = self.bb('QQQ', 20, 2)
        
        # Risk management
        self.stop_loss = 0.08109233440468619
        self.take_profit = 0.1019298498016043
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.4960967520708297 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("QQQ", 1.4960967520708297 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08109233440468619:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.1019298498016043:
                self.liquidate()
