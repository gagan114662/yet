"""
Simple High-Performance Strategy
A straightforward strategy focused on generating high returns with frequent trading
"""
import numpy as np
...
class SimpleHighPerformanceStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        # Log initialization
        self.Log("Simple High-Performance Strategy initialized")
    ...
    def RebalancePortfolio(self):
        ...
        for symbol, signal in sorted_symbols[:self.max_positions]:
            if abs(signal) > 0.2:  # Only trade on meaningful signals
                ...
                if current_position == 0:
                    ...
                    # Calculate position size (more aggressive for stronger signals)
                    size = self.position_size * min(abs(signal) * 2, 1.5)
