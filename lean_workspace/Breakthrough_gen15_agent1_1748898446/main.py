...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen15_agent1
    Type: MEAN_REVERSION | Asset: TQQQ | Leverage: 10.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(10.376508777147523)
        ...
        self.sma_slow = self.sma('TQQQ', 40)
        self.rsi = self.rsi('TQQQ', 20)
        self.bb = self.bb('TQQQ', 20, 2)
        self.atr = self.atr('TQQQ', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.07558660660043634
        self.take_profit = 0.2378464827962739
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.0406574158959354
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07558660660043634:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.2378464827962739:
                self.liquidate()
