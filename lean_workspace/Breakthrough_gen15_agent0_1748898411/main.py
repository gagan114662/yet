...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen15_agent0
    Type: MOMENTUM | Asset: QQQ | Leverage: 17.2x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(17.16943151989077)
        ...
        self.sma_slow = self.sma('QQQ', 22)
        self.rsi = self.rsi('QQQ', 16)
        self.atr = self.atr('QQQ', 14)
        
        # Risk management
        self.stop_loss = 0.051637193817632904
        self.take_profit = 0.20588657244856717
    ...
    def on_data(self, data):
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.051637193817632904:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.20588657244856717:
                self.liquidate()
