...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen7_agent4
    Type: BREAKOUT | Asset: UPRO | Leverage: 10.9x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(10.926073770756949)
        ...
        self.sma_slow = self.sma('UPRO', 45)
        self.rsi = self.rsi('UPRO', 11)
        self.macd = self.macd('UPRO', 12, 26, 9)
        self.bb = self.bb('UPRO', 20, 2)
        
        # Risk management
        self.stop_loss = 0.0566292032871603
        self.take_profit = 0.15772189649400442
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.1179009128478024
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.0566292032871603:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.15772189649400442:
                self.liquidate()
