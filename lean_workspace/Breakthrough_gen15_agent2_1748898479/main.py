...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen15_agent2
    Type: BREAKOUT | Asset: QQQ | Leverage: 12.6x
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.8919469038936352
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.057107966736048024:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.0981897608330181:
                self.liquidate()
