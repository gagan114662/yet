...
class VandanStrategyBurke(QCAlgorithm):
    ...
    def get_market_strength(self):
        ...
        if self.Time.minute == 0 and not any(x.Invested for x in self.Portfolio.Values):
            self.Debug(f"[{self.Time}|${current_price:.2f}] SIGNALS - "
                   f"Range: {range_signal:.2f}, Momentum: {momentum_signal:.2f}, Final: {final_signal:.2f}")
    ...
    def manage_positions(self, slice, market_strength, remaining_bars_atr) -> None:
        ...
        for holding in self.Portfolio.Values:
            if holding.Invested and holding.Type == SecurityType.Option and holding.Quantity < 0:
                ...
                if contract and contract[0].AskPrice > self.conditional_exit_price:
                    ...
                    exit_order = self.LimitOrder(option, -holding.Quantity, self.conditional_exit_price, tag=f"[SPY: {slice.Bars[self.underlying_symbol].Price:.2f}|Mkt:{round(market_strength, 2)}|ATR:{remaining_bars_atr:.2f}]")
    ...
    def EODClosePositions(self, slice: Slice) -> None:
        ...
        if self.Time.hour != 15 or self.Time.minute != 59:
            return
        ...
        for holding in self.Portfolio.Values:
            ...
            if chain:
                ...
                if chain_contract:
                    ...
                    order = self.MarketOrder(symbol, -quantity, True)  # True for aggressive order
                    
                    if order.Status == OrderStatus.Filled:
                        fill_price = self.Securities[symbol].Price
                        self.Debug(f"[{self.Time}|${underlying_price:.2f}] Closed {symbol} ({quantity} contracts) @ ${fill_price:.2f} (Market: ${market_price:.2f})")
                    else:
                        self.PrintOutput(f"Failed to close {symbol} - Status: {order.Status}", print=False)
    ...
    def OnData(self, slice: Slice) -> None:
        ...
        self.manage_positions(slice, market_strength, remaining_bars_atr)  # Check positions for exit conditions
