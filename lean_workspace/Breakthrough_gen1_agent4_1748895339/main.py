...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen1_agent4
    Type: MEAN_REVERSION | Asset: UPRO | Leverage: 5.8x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.761337678075067)
        ...
        self.sma_slow = self.sma('UPRO', 43)
        ...
        # Risk management
        self.stop_loss = 0.02202631513849405
        self.take_profit = 0.19254845926482322
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.9470490710321835
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.02202631513849405:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.19254845926482322:
                self.liquidate()
