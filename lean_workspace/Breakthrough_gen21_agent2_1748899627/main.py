...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen21_agent2
    Type: BREAKOUT | Asset: UPRO | Leverage: 8.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(8.269282003831192)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('UPRO', 3)
        self.sma_slow = self.sma('UPRO', 23)
        self.rsi = self.rsi('UPRO', 15)
        self.bb = self.bb('UPRO', 20, 2)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.08658695998841491
        self.take_profit = 0.10719727317928561
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.4475697498825904
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08658695998841491:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.10719727317928561:
                self.liquidate()
