...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen13_agent4
    Type: MOMENTUM | Asset: TQQQ | Leverage: 6.7x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(6.67221899235755)
        ...
        self.sma_slow = self.sma('TQQQ', 43)
        self.atr = self.atr('TQQQ', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.09905580205198669
        self.take_profit = 0.07932845435207908
    ...
    def on_data(self, data):
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.09905580205198669:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.07932845435207908:
                self.liquidate()
