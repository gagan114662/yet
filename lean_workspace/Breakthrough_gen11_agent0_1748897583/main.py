...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen11_agent0
    Type: HYBRID | Asset: BITO | Leverage: 8.8x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(8.812501396504084)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('BITO', 5)
        self.sma_slow = self.sma('BITO', 48)
        self.rsi = self.rsi('BITO', 19)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.08612942975673502
        self.take_profit = 0.23871736806477312
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.754907606556373 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("BITO", 1.754907606556373 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08612942975673502:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.23871736806477312:
                self.liquidate()
