...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen4_agent2
    Type: BREAKOUT | Asset: BITO | Leverage: 5.8x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.766309991712548)
        ...
        self.rsi = self.rsi('BITO', 20)
        self.macd = self.macd('BITO', 12, 26, 9)
        self.atr = self.atr('BITO', 14)
        
        # Risk management
        self.stop_loss = 0.04844060944383542
        self.take_profit = 0.12403494450377367
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.66458799567892
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.04844060944383542:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.12403494450377367:
                self.liquidate()
