...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen5_agent3
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(12.436174943617477)
        ...
        self.sma_slow = self.sma('BITO', 36)
        self.rsi = self.rsi('BITO', 19)
        self.macd = self.macd('BITO', 12, 26, 9)
        self.atr = self.atr('BITO', 14)
        
        # Risk management
        self.stop_loss = 0.0987009491618844
        self.take_profit = 0.2442370950817605
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.2592997036407432 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("BITO", 1.2592997036407432 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.0987009491618844:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.2442370950817605:
                self.liquidate()
