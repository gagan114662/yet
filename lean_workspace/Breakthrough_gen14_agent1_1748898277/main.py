...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen14_agent1
    Type: HYBRID | Asset: BITO | Leverage: 18.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(18.406222634108854)
        ...
        self.sma_slow = self.sma('BITO', 47)
        self.rsi = self.rsi('BITO', 16)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.07313599970652279
        self.take_profit = 0.12803318690032744
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.9189494171082742 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("BITO", 1.9189494171082742 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07313599970652279:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.12803318690032744:
                self.liquidate()
