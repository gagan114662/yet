...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen14_agent2
    Type: HYBRID | Asset: UPRO | Leverage: 6.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(6.251410090138047)
        ...
        self.sma_slow = self.sma('UPRO', 33)
        self.rsi = self.rsi('UPRO', 17)
        self.macd = self.macd('UPRO', 12, 26, 9)
        
        # Risk management
        self.stop_loss = 0.03887102140942815
        self.take_profit = 0.24072900859684015
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 2.1271117387262084 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("UPRO", 2.1271117387262084 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.03887102140942815:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.24072900859684015:
                self.liquidate()
