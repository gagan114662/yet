...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen1_agent1
    Type: MEAN_REVERSION | Asset: UPRO | Leverage: 5.1x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.051374932424795)
        ...
        self.sma_slow = self.sma('UPRO', 31)
        self.macd = self.macd('UPRO', 12, 26, 9)
        self.bb = self.bb('UPRO', 20, 2)
        self.atr = self.atr('UPRO', 14)
        
        # Risk management
        self.stop_loss = 0.0563778565659637
        self.take_profit = 0.12063165675100572
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.915649900349108
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.0563778565659637:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.12063165675100572:
                self.liquidate()
