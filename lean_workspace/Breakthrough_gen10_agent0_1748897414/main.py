...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen10_agent0
    Type: MOMENTUM | Asset: QQQ | Leverage: 14.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(14.446879686067204)
        ...
        self.rsi = self.rsi('QQQ', 18)
        self.atr = self.atr('QQQ', 14)
        
        # Risk management
        self.stop_loss = 0.02233546721829198
        self.take_profit = 0.15178911150822694
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.1167815872826896
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.02233546721829198:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.15178911150822694:
                self.liquidate()
