...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen13_agent3
    Type: MEAN_REVERSION | Asset: TQQQ | Leverage: 11.6x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(11.590004834127374)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('TQQQ', 12)
        self.sma_slow = self.sma('TQQQ', 23)
        self.rsi = self.rsi('TQQQ', 17)
        self.bb = self.bb('TQQQ', 20, 2)
        self.atr = self.atr('TQQQ', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.06144938623988064
        self.take_profit = 0.061055573743720865
    ...
    def on_data(self, data):
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.06144938623988064:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.061055573743720865:
                self.liquidate()
