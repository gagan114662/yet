...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen3_agent0
    Type: MEAN_REVERSION | Asset: SPY | Leverage: 5.1x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.060001475004613)
        ...
        # Risk management
        self.stop_loss = 0.05101005504872415
        self.take_profit = 0.23870717014807213
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.8803199127854104
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05101005504872415:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.23870717014807213:
                self.liquidate()
