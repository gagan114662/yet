...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen10_agent2
    Type: BREAKOUT | Asset: BITO | Leverage: 18.0x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(18.022927975337165)
        ...
        self.sma_slow = self.sma('BITO', 24)
        self.bb = self.bb('BITO', 20, 2)
        self.atr = self.atr('BITO', 14)
        
        # Risk management
        self.stop_loss = 0.029857639833734614
        self.take_profit = 0.228010114246458
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.718834670711003
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.029857639833734614:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.228010114246458:
                self.liquidate()
