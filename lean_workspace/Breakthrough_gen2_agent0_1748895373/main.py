...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen2_agent0
    Type: MOMENTUM | Asset: UVXY | Leverage: 6.0x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.964829839304394)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('UVXY', 12)
        self.sma_slow = self.sma('UVXY', 34)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.07074922936270298
        self.take_profit = 0.14002592832450733
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.4638130980007347
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07074922936270298:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.14002592832450733:
                self.liquidate()
