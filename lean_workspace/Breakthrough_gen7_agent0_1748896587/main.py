...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen7_agent0
    Type: MOMENTUM | Asset: BITO | Leverage: 14.6x
    ...
    def initialize(self):
        ...
        # Risk management
        self.stop_loss = 0.09334308257706976
        self.take_profit = 0.1629239218510087
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 1.29200636685392
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.09334308257706976:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.1629239218510087:
                self.liquidate()
