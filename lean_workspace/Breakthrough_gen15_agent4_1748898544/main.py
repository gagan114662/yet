...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen15_agent4
    Type: HYBRID | Asset: UVXY | Leverage: 11.2x
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.539550635745391 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("UVXY", 1.539550635745391 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05612777176787881:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.05682525459663881:
                self.liquidate()
