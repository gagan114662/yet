...
class OptionsStrategyDev(QCAlgorithm): 
    ...
    def on_data(self, slice: Slice) -> None:
        ...
        if self.print_bool:
            self.PrintOutput(f"{self.Time}| Min Expiry: {expiry}")
        ...
        if self.print_bool:
            self.PrintOutput({'calls': len(calls), 'puts': len(puts)})
            for i, put in enumerate(puts):
                self.PrintOutput(f"{self.Time}| Put {i}: {put.symbol} | Strike: {put.strike} | IV: {put.implied_volatility:.2f} | $: {put.last_price} | SPY: {chain.underlying.price}")
            
            for i, call in enumerate(calls):
                self.PrintOutput(f"{self.Time}| Call {i}: {call.symbol} | Strike: {call.strike} | IV: {call.implied_volatility:.2f} | $: {call.last_price} | SPY: {chain.underlying.price}")
        ...
        if not valid_puts:
            if self.print_bool:
                self.PrintOutput("No valid puts found matching criteria")
            return
            
        put = sorted(valid_puts, key = lambda x: x.strike)[0]  # Get the lowest strike from valid puts

        if self.print_bool:
            self.PrintOutput(f"Selected put: Strike: {put.strike}, Price: {put.last_price}, Distance from underlying: {((chain.underlying.price - put.strike)/chain.underlying.price)*100:.2f}%")
