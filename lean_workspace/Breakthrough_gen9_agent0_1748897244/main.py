...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen9_agent0
    Type: MOMENTUM | Asset: UVXY | Leverage: 4.9x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(4.886878884832968)
        ...
        self.sma_slow = self.sma('UVXY', 26)
        self.rsi = self.rsi('UVXY', 19)
        self.macd = self.macd('UVXY', 12, 26, 9)
        self.bb = self.bb('UVXY', 20, 2)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.05239971277094582
        self.take_profit = 0.05971330621037385
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.1740709395321227
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05239971277094582:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.05971330621037385:
                self.liquidate()
