...
class HighPerformanceStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.SetStartDate(2018, 1, 1)  # Start in 2018 to include COVID crash and recovery
        ...
        # Universe selection - focus on high-volatility leveraged ETFs for maximum returns
        # These assets have extreme volatility which can generate high returns when traded correctly
        self.symbols = [
            # Leveraged ETFs (3x)
            "TQQQ", "SQQQ", "UPRO", "SPXU", "TNA", "TZA", "LABU", "LABD", "NUGT", "DUST",
            "FAS", "FAZ", "ERX", "ERY", "YINN", "YANG", "SOXL", "SOXS", "TECL", "TECS",
            # Regular ETFs for diversification
            "SPY", "QQQ", "IWM", "EEM", "GLD", "TLT", "USO", "UNG", "XLF", "XLE"
        ]
        self.data = {}
        ...
        for symbol, score in sorted_symbols[:self.max_positions]:
            if score > 0.7:  # Only invest in strong signals
                ...
                if current_position == 0:
                    ...
                    # Adjust for market regime - more aggressive sizing
                    if self.market_regime == "bearish":
                        # In bearish markets, focus on inverse ETFs
                        if symbol in ["SQQQ", "SPXU", "TZA", "LABD", "DUST", "FAZ", "ERY", "YANG", "SOXS", "TECS"]:
                            size *= 1.5  # Increase position size for inverse ETFs in bear markets
                            size = min(size, 0.5)  # Cap at 50%
                        else:
                            size *= 0.3  # Reduce position size for long ETFs in bear markets
                    elif self.market_regime == "bullish":
                        # In bullish markets, focus on long leveraged ETFs
                        if symbol in ["TQQQ", "UPRO", "TNA", "LABU", "NUGT", "FAS", "ERX", "YINN", "SOXL", "TECL"]:
                            size *= 1.5  # Increase position size for leveraged ETFs in bull markets
                            size = min(size, 0.5)  # Cap at 50%
                        else:
                            size *= 1.2  # Slightly increase position size for other ETFs
