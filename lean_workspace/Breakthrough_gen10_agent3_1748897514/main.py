...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen10_agent3
    Type: MOMENTUM | Asset: TQQQ | Leverage: 6.2x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(6.187547838287241)
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.794833376050652
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08864362719040293:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.12329023559787684:
                self.liquidate()
