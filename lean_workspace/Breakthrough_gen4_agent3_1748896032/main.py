...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen4_agent3
    Type: HYBRID | Asset: BITO | Leverage: 11.5x
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.6917640987126012 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("BITO", 1.6917640987126012 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07171750159832714:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.07878806864410662:
                self.liquidate()
