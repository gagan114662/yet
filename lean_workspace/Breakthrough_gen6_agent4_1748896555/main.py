...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen6_agent4
    Type: MEAN_REVERSION | Asset: QQQ | Leverage: 10.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(10.435863815990341)
        ...
        self.sma_slow = self.sma('QQQ', 41)
        self.macd = self.macd('QQQ', 12, 26, 9)
        self.bb = self.bb('QQQ', 20, 2)
        
        # Risk management
        self.stop_loss = 0.028762577022006866
        self.take_profit = 0.24435013662494515
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 1.266108556054593
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.028762577022006866:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.24435013662494515:
                self.liquidate()
