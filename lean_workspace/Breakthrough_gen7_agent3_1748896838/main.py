...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen7_agent3
    Type: BREAKOUT | Asset: QQQ | Leverage: 14.4x
    ...
    def initialize(self):
        ...
        # Risk management
        self.stop_loss = 0.028966793025356916
        self.take_profit = 0.17124333999081448
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.315565359678178
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.028966793025356916:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.17124333999081448:
                self.liquidate()
