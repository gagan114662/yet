...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen3_agent2
    Type: HYBRID | Asset: BITO | Leverage: 19.8x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(19.80683735992544)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('BITO', 13)
        self.sma_slow = self.sma('BITO', 35)
        self.atr = self.atr('BITO', 14)
        
        # Risk management
        self.stop_loss = 0.07750464784718268
        self.take_profit = 0.09040513044357937
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.9557343545547072 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("BITO", 1.9557343545547072 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07750464784718268:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.09040513044357937:
                self.liquidate()
