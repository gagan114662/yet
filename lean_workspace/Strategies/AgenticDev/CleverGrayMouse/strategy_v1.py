...
class CryptoSwingTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.symbol = self.AddCrypto("ETHUSD", QuantConnect.Resolution.Minute).Symbol  # Correct way to add crypto
        ...
        # Indicators
        self.macd = self.MACD(self.symbol, self.fast_period, self.slow_period, self.signal_period, MovingAverageType.Exponential, QuantConnect.Resolution.Minute)
        self.rsi = self.RSI(self.symbol, self.rsi_period, MovingAverageType.Exponential, QuantConnect.Resolution.Minute)
    ...
    def SetStopLoss(self, symbol, percentage):
        ...
        self.stop_market_ticket = self.StopMarketOrder(symbol, -self.Portfolio[symbol].Quantity, stop_price)
    ...
    def SetProfitTarget(self, symbol, percentage):
         ...
         self.take_profit_ticket = self.LimitOrder(symbol, -self.Portfolio[symbol].Quantity, take_profit_price)
