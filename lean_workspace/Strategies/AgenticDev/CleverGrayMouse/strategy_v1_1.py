...
class CryptoSwingTradingAlgorithm(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        #self.symbol = Symbol.Create("ETHUSD", SecurityType.Crypto, Market.GDAX) # This is incorrect
        #self.AddCrypto(self.symbol, Resolution.Minute)  # Minute data for more frequent trading
        ...
        self.long_stop_loss_order_event_id = None
        self.long_take_profit_order_event_id = None
        self.short_stop_loss_order_event_id = None
        self.short_take_profit_order_event_id = None
    ...
    def LongEntry(self):
        ...
        self.SetStopLoss(self.symbol, self.stop_loss_percentage, is_long=True)
        self.SetProfitTarget(self.symbol, self.take_profit_percentage, is_long=True)
    ...
    def ShortEntry(self):
        ...
        # Set Stop Loss and Take Profit
        self.SetStopLoss(self.symbol, self.stop_loss_percentage, is_long=False)
        self.SetProfitTarget(self.symbol, self.take_profit_percentage, is_long=False)
    ...
    def CancelOrders(self):
        if self.stop_market_ticket is not None and not self.stop_market_ticket.Status.IsClosed:
            ...
            self.stop_market_ticket = None  # Reset the ticket

        if self.take_profit_ticket is not None and not self.take_profit_ticket.Status.IsClosed:
            self.take_profit_ticket.Cancel()
            self.take_profit_ticket = None  # Reset the ticket

    def SetStopLoss(self, symbol, percentage, is_long):
        """Sets a stop loss order."""
        if is_long:
            stop_price = self.Securities[symbol].Close * (1 - percentage)
            order_properties = OrderProperties()
            order_properties.TimeInForce = TimeInForce.GoodTillCanceled
            
            if self.long_stop_loss_order_event_id is not None:
                self.Liquidate(symbol, "Updating Long Stop Loss")
                
            event = self.StopMarketOrder(symbol, -self.Portfolio[symbol].Quantity, stop_price,  orderProperties=order_properties)
            self.long_stop_loss_order_event_id = event.OrderId

            self.Debug(f"Long Stop Loss set for {symbol} at {stop_price}")
            ...
            if self.short_stop_loss_order_event_id is not None:
                self.Liquidate(symbol, "Updating Short Stop Loss")
                
            event = self.StopMarketOrder(symbol, -self.Portfolio[symbol].Quantity, stop_price,  orderProperties=order_properties)
            self.short_stop_loss_order_event_id = event.OrderId
            self.Debug(f"Short Stop Loss set for {symbol} at {stop_price}")


    def SetProfitTarget(self, symbol, percentage, is_long):
         ...
         if is_long:
             ...
             if self.long_take_profit_order_event_id is not None:
                self.Liquidate(symbol, "Updating Long Take Profit")
             
             event = self.LimitOrder(symbol, -self.Portfolio[symbol].Quantity, take_profit_price, orderProperties=order_properties)
             self.long_take_profit_order_event_id = event.OrderId
             self.Debug(f"Long Take Profit set for {symbol} at {take_profit_price}")
             ...
             if self.short_take_profit_order_event_id is not None:
                self.Liquidate(symbol, "Updating Short Take Profit")
             
             event = self.LimitOrder(symbol, -self.Portfolio[symbol].Quantity, take_profit_price, orderProperties=order_properties)
             self.short_take_profit_order_event_id = event.OrderId
             self.Debug(f"Short Take Profit set for {symbol} at {take_profit_price}")
