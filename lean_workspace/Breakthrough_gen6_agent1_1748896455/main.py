...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen6_agent1
    Type: MOMENTUM | Asset: BITO | Leverage: 17.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(17.44265017876681)
        ...
        # Risk management
        self.stop_loss = 0.08314250825926967
        self.take_profit = 0.1389145574139869
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 1.618703916435938
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08314250825926967:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.1389145574139869:
                self.liquidate()
