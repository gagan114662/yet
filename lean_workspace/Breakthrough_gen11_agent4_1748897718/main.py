...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen11_agent4
    Type: MEAN_REVERSION | Asset: UVXY | Leverage: 7.0x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(7.041510883353292)
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.7195923742927404
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07550761868087402:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.2336970869838803:
                self.liquidate()
