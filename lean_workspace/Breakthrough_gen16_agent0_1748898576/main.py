...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen16_agent0
    Type: HYBRID | Asset: QQQ | Leverage: 19.7x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(19.65543662647302)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('QQQ', 4)
        self.sma_slow = self.sma('QQQ', 49)
        self.macd = self.macd('QQQ', 12, 26, 9)
        self.bb = self.bb('QQQ', 20, 2)
        self.atr = self.atr('QQQ', 14)
        
        # Risk management
        self.stop_loss = 0.043042101129130546
        self.take_profit = 0.14632007458554241
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.5346283051043663 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("QQQ", 1.5346283051043663 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.043042101129130546:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.14632007458554241:
                self.liquidate()
