...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen11_agent1
    Type: MEAN_REVERSION | Asset: QQQ | Leverage: 8.5x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(8.480920959325115)
        ...
        self.sma_slow = self.sma('QQQ', 36)
        self.rsi = self.rsi('QQQ', 16)
        self.atr = self.atr('QQQ', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.07995345545174008
        self.take_profit = 0.21917875050903712
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.7022615736078475
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07995345545174008:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.21917875050903712:
                self.liquidate()
