...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen7_agent1
    Type: MOMENTUM | Asset: QQQ | Leverage: 5.9x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.853345686814275)
        ...
        self.sma_slow = self.sma('QQQ', 24)
        self.rsi = self.rsi('QQQ', 20)
        self.bb = self.bb('QQQ', 20, 2)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.08719937621942037
        self.take_profit = 0.053788189084975006
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 1.298961635568409
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08719937621942037:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.053788189084975006:
                self.liquidate()
