...
class Enhanced_High_Return_Strategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.max_position_size = 0.4  # 40% per position for higher returns
        self.leverage_multiplier = 2.5  # 250% total exposure
        
        # Schedule daily rebalancing for more trades
    ...
    def UpdateMarketRegime(self):
        ...
        spy_rsi = self.indicators["SPY"]["rsi_fast"].Current.Value
    ...
    def ExecuteEnhancedTrades(self, signals):
        ...
        if not strong_signals:
            ...
            for symbol in self.symbols:
                if self.Portfolio[symbol].Invested:
                    ...
                    if abs(current_weight) > 0.1:  # Reduce large positions
        ...
        base_leverage = {
            "BULL": 2.5,     # Very high leverage in bull markets
            "NEUTRAL": 2.0,  # High leverage in neutral markets
            "BEAR": 1.5      # Still leveraged in bear markets for short opportunities
        }.get(self.market_regime, 1.8)
        ...
        for symbol, signal in strong_signals.items():
            ...
            position_size = signal * signal_weight * base_leverage * 0.85  # Use 85% of capacity
        ...
        for symbol in self.symbols:
            if symbol in data and self.indicators[symbol]["momentum_fast"].IsReady:
                ...
                if abs(mom_fast) > 0.015:  # Strong momentum
                    ...
                    if abs(current_weight) < 0.15:
                        quick_position = 0.15 if mom_fast > 0 else -0.15  # Quick positions
                        ...
                        if abs(new_weight - current_weight) > 0.03:  # Only if significant change
                            self.SetHoldings(symbol, new_weight)
                            self.trade_count += 1
