...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen7_agent2
    Type: HYBRID | Asset: UPRO | Leverage: 4.4x
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 1.5860543452133673 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("UPRO", 1.5860543452133673 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.06096700041653609:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.20357166332089704:
                self.liquidate()
