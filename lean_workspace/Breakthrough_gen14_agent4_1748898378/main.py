...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen14_agent4
    Type: BREAKOUT | Asset: TQQQ | Leverage: 11.5x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(11.502604877926698)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('TQQQ', 5)
        self.sma_slow = self.sma('TQQQ', 48)
        self.bb = self.bb('TQQQ', 20, 2)
        self.atr = self.atr('TQQQ', 14)
        
        # Risk management
        self.stop_loss = 0.04642355469757298
        self.take_profit = 0.1558945887052018
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.5045433578362437
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.04642355469757298:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.1558945887052018:
                self.liquidate()
