...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen2_agent3
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(3.7024977694727457)
        ...
        self.sma_slow = self.sma('TQQQ', 49)
        self.macd = self.macd('TQQQ', 12, 26, 9)
        
        # Risk management
        self.stop_loss = 0.031653324484469406
        self.take_profit = 0.24185994699928365
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.085833305958367
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.031653324484469406:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.24185994699928365:
                self.liquidate()
