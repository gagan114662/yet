...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen5_agent1
    Type: MEAN_REVERSION | Asset: TQQQ | Leverage: 8.6x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(8.580043089703796)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('TQQQ', 9)
        self.sma_slow = self.sma('TQQQ', 32)
        self.rsi = self.rsi('TQQQ', 15)
        ...
        # Risk management
        self.stop_loss = 0.05897034697313325
        self.take_profit = 0.0962706975083153
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.1433044451864003
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05897034697313325:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.0962706975083153:
                self.liquidate()
