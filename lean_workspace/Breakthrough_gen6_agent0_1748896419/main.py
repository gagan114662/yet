...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen6_agent0
    Type: BREAKOUT | Asset: QQQ | Leverage: 11.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(11.292993644509352)
        ...
        # Risk management
        self.stop_loss = 0.04389378866853022
        self.take_profit = 0.16907087739184423
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.057830806350315
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.04389378866853022:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.16907087739184423:
                self.liquidate()
