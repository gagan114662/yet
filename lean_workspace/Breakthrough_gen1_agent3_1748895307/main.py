...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen1_agent3
    Type: MOMENTUM | Asset: SPY | Leverage: 11.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(11.272447015708568)
        ...
        self.rsi = self.rsi('SPY', 18)
        self.atr = self.atr('SPY', 14)
        
        # Risk management
        self.stop_loss = 0.06672523795839343
        self.take_profit = 0.23583361154605514
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 1.358575162484562
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.06672523795839343:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.23583361154605514:
                self.liquidate()
