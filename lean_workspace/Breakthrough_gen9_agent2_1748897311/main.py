...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen9_agent2
    Type: MOMENTUM | Asset: BITO | Leverage: 19.8x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(19.77533010740838)
        ...
        self.sma_slow = self.sma('BITO', 50)
        ...
        # Risk management
        self.stop_loss = 0.08707012587007969
        self.take_profit = 0.16766923324296673
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 1.769107017865258
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.08707012587007969:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.16766923324296673:
                self.liquidate()
