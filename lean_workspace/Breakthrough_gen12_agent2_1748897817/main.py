...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen12_agent2
    Type: BREAKOUT | Asset: UPRO | Leverage: 10.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(10.437444480273516)
        ...
        self.sma_slow = self.sma('UPRO', 37)
        self.bb = self.bb('UPRO', 20, 2)
        self.atr = self.atr('UPRO', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.07077371782203436
        self.take_profit = 0.175884194930169
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.5278830689334058
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.07077371782203436:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.175884194930169:
                self.liquidate()
