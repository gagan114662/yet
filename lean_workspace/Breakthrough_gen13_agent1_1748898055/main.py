...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen13_agent1
    Type: BREAKOUT | Asset: TQQQ | Leverage: 3.7x
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.176731003041869
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.04564920502662718:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.11006760304636312:
                self.liquidate()
