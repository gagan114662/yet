...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen3_agent3
    Type: HYBRID | Asset: TQQQ | Leverage: 3.4x
    ...
    def initialize(self):
        ...
        # Risk management
        self.stop_loss = 0.0900342649920859
        self.take_profit = 0.12480489351701896
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 2.7342039421513094 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("TQQQ", 2.7342039421513094 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.0900342649920859:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.12480489351701896:
                self.liquidate()
