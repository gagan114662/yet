...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen15_agent3
    Type: MEAN_REVERSION | Asset: UPRO | Leverage: 4.0x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(3.9949562251015864)
        ...
        # Risk management
        self.stop_loss = 0.06748975290052621
        self.take_profit = 0.14127894891032278
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.1580145659981236
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.06748975290052621:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.14127894891032278:
                self.liquidate()
