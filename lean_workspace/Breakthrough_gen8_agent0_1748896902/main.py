...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen8_agent0
    Type: BREAKOUT | Asset: BITO | Leverage: 8.8x
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.026090276695645
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.04227727547116164:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.057948509242313075:
                self.liquidate()
