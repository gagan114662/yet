...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen12_agent0
    Type: BREAKOUT | Asset: TQQQ | Leverage: 4.5x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(4.454059133396764)
        ...
        self.rsi = self.rsi('TQQQ', 18)
        self.macd = self.macd('TQQQ', 12, 26, 9)
        
        # Risk management
        self.stop_loss = 0.09425292895771295
        self.take_profit = 0.10495769821161019
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 2.717833261729778
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.09425292895771295:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.10495769821161019:
                self.liquidate()
