...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen10_agent4
    Type: MOMENTUM | Asset: UVXY | Leverage: 7.9x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(7.893092687049386)
        ...
        self.sma_slow = self.sma('UVXY', 47)
        self.rsi = self.rsi('UVXY', 16)
        self.macd = self.macd('UVXY', 12, 26, 9)
        self.bb = self.bb('UVXY', 20, 2)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.05820161783132809
        self.take_profit = 0.17833802832629614
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.923691928047604
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.05820161783132809:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.17833802832629614:
                self.liquidate()
