...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen8_agent4
    Type: MOMENTUM | Asset: UPRO | Leverage: 5.4x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.442429961160376)
        ...
        # Risk management
        self.stop_loss = 0.09498356856381467
        self.take_profit = 0.19027030752926316
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 2.054995747537638
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.09498356856381467:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.19027030752926316:
                self.liquidate()
