...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen1_agent0
    Type: MEAN_REVERSION | Asset: TQQQ | Leverage: 5.5x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(5.4691340410492435)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('TQQQ', 14)
        self.sma_slow = self.sma('TQQQ', 45)
        self.atr = self.atr('TQQQ', 14)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.03557271040430739
        self.take_profit = 0.17214318968504683
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.6040687844464383
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.03557271040430739:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.17214318968504683:
                self.liquidate()
