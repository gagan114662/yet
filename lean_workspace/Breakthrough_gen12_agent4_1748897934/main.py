...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen12_agent4
    Type: MEAN_REVERSION | Asset: BITO | Leverage: 11.6x
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 1.318144509326623
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.0478315129321207:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.12845018160639443:
                self.liquidate()
