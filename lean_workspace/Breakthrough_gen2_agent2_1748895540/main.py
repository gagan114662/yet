...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen2_agent2
    Type: HYBRID | Asset: SPY | Leverage: 6.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(6.291812290213548)
        
        # Initialize tracking for Sharpe calculation
        self.daily_returns = []
        self.last_portfolio_value = self.portfolio.total_portfolio_value
        
        self.sma_fast = self.sma('SPY', 7)
        self.sma_slow = self.sma('SPY', 39)
        self.bb = self.bb('SPY', 20, 2)
        
        # Risk management
        self.stop_loss = 0.038015999047681326
        self.take_profit = 0.11236211111486516
    ...
    def on_data(self, data):
        ...
        if momentum_signal:
            # Momentum phase
            position_size = 2.695761061498799 * 0.8
            ...
            if current_price < sma_mid * 0.97:
                self.set_holdings("SPY", 2.695761061498799 * 0.6)
                self.entry_price = current_price
            else:
                self.liquidate()
                self.entry_price = 0
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.038015999047681326:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.11236211111486516:
                self.liquidate()
