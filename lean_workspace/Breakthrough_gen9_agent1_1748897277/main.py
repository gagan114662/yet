...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen9_agent1
    Type: MOMENTUM | Asset: UVXY | Leverage: 8.4x
    ...
    def on_data(self, data):
        ...
        if self.sma_fast.current.value > self.sma_slow.current.value:
            ...
            position_size = 1.568956080880974
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.039303507859132836:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.24632379053920106:
                self.liquidate()
