...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen12_agent1
    Type: MEAN_REVERSION | Asset: BITO | Leverage: 15.8x
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 1.8843557494857988
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.029720920091258575:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.24159270022550094:
                self.liquidate()
