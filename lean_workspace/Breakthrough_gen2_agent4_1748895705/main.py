...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen2_agent4
    Type: BREAKOUT | Asset: BITO | Leverage: 16.1x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(16.097933744203345)
        ...
        self.sma_slow = self.sma('BITO', 46)
        self.vix = self.add_index('VIX', Resolution.Daily)
        
        # Risk management
        self.stop_loss = 0.04088359506456958
        self.take_profit = 0.2469894021414863
    ...
    def on_data(self, data):
        ...
        if current_price > high_20 * 1.01:
            position_size = 1.7030233321142774
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.04088359506456958:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.2469894021414863:
                self.liquidate()
