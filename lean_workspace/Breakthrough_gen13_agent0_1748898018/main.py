...
class BreakthroughStrategy(QCAlgorithm):
    """
    Breakthrough Strategy - Agent: gen13_agent0
    Type: MEAN_REVERSION | Asset: TQQQ | Leverage: 6.3x
    ...
    def initialize(self):
        ...
        self.symbol.set_leverage(6.278381017588321)
        ...
        self.sma_slow = self.sma('TQQQ', 24)
        ...
        # Risk management
        self.stop_loss = 0.09134901483211022
        self.take_profit = 0.23644547600687982
    ...
    def on_data(self, data):
        ...
        if current_price < sma_mid * 0.98:
            position_size = 2.4286588596131846
        ...
        if self.portfolio.invested and self.entry_price > 0:
            ...
            # Stop loss
            if pnl_pct < -0.09134901483211022:
                self.liquidate()
                self.log(f"Stop loss triggered: {pnl_pct:.2%}")
            
            # Take profit
            elif pnl_pct > 0.23644547600687982:
                self.liquidate()
