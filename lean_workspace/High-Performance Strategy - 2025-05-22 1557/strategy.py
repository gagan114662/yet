...
class HighPerformanceStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        # Set start and end dates for backtest - use a period that includes both bull and bear markets
        self.SetStartDate(2020, 1, 1)  # Start in 2020 to include COVID crash and recovery
        self.SetEndDate(2023, 12, 31)  # End in 2023 to ensure we have complete data
        self.SetCash(100000)
        
        # Set brokerage model
        self.SetBrokerageModel(BrokerageModel.QuantConnectBrokerage())
        
        # Universe selection - focus on a smaller set of well-known ETFs with good data availability
        # Include both regular and leveraged ETFs for better performance
        self.symbols = [
            # Core ETFs with good liquidity
            "SPY", "QQQ", "IWM", "EEM", "GLD", "TLT", "XLF", "XLE", "XLK", "XLV",
            # Popular leveraged ETFs with reliable data
        ...
        for symbol in self.symbols:
            ...
            security.SetLeverage(self.leverage_factor)  # Use maximum leverage to boost returns
        ...
        # Strategy parameters - extremely aggressive to meet high performance targets
        self.rebalance_days = 1  # Rebalance daily to increase trade frequency
        self.max_positions = 8  # Maximum number of positions at any time
        self.position_size = 0.30  # Base position size (30% of portfolio)
        self.stop_loss_pct = 0.10  # 10% stop loss
        self.trailing_stop_pct = 0.12  # 12% trailing stop
        self.profit_target_pct = 0.20  # 20% profit target
        self.leverage_factor = 3  # Use maximum leverage
    ...
    def RebalancePortfolio(self):
        ...
        # Force daily rebalancing to increase trade frequency
        ...
        self.Log("Top scoring symbols:")
        for symbol, score in sorted_symbols[:5]:
            self.Log(f"  {symbol}: {score:.4f}")
        ...
        for symbol, score in sorted_symbols[:self.max_positions]:
            if score > 0.3:  # Lower threshold to ensure more trades
                ...
                if current_position == 0:
                    ...
                    # Adjust for market regime - simplified and more aggressive sizing
                    if self.market_regime == "bearish":
                        # In bearish markets, focus on inverse ETFs
                        if symbol in ["SQQQ", "SPXU", "TZA"]:
                            size *= 2.0  # Increase position size for inverse ETFs in bear markets
                            size = min(size, 0.6)  # Cap at 60%
                        else:
                            size *= 0.5  # Reduce position size for long ETFs in bear markets
                    elif self.market_regime == "bullish":
                        # In bullish markets, focus on long leveraged ETFs
                        if symbol in ["TQQQ", "UPRO", "TNA"]:
                            size *= 2.0  # Increase position size for leveraged ETFs in bull markets
                            size = min(size, 0.6)  # Cap at 60%
                        else:
                            size *= 1.5  # Increase position size for other ETFs
                    else:
                        # In neutral markets, still be aggressive
                        size *= 1.2
